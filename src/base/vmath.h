...
public:
	...
	vector2_base negate() const { return vector2_base(-x, -y); } // luabridge doesn't support parameter overloading
	...
	{
		char aBuf[64];
		str_format(aBuf, sizeof(aBuf), "(%.4f|%.4f)", x,y);
...
public:
	...
	{
		char aBuf[64];
		str_format(aBuf, sizeof(aBuf), "(%.4f|%.4f|%.4f)", x,y,z);
		return std::string(aBuf);
	}
};
...
public:
	...
	{
		char aBuf[64];
		str_format(aBuf, sizeof(aBuf), "(%.4f|%.4f|%.4f|%.4f)", r,g,b,a);
		return std::string(aBuf);
