...
luabridge::LuaRef CJsonValue::ToTable(lua_State *L)
{
	...
	luabridge::LuaRef Result = luabridge::Stack<luabridge::LuaRef>::get(L, -1);
	lua_pop(L, 1);
	return Result;
}
...
static json_value *ConvertSomething(const luabridge::LuaRef& data)
{
	...
	switch(data.type())
	{
		...
		case LUA_TTABLE:
		{
			// find out whether the table is ipairs-iteratable (yes: array, no: object)
			bool IsArray = true;
...
CJsonValue CLuaJson::Convert(luabridge::LuaRef data)
{
	CJsonValue Result;
