...
def get_padding(string, padding):
	return ' '*max(1, padding-len(string))

def create_lua_constants(names, num, padding):
	lines = []
	i = 0
	for name in names:
		lines += ["%s%s= %i" % (name, get_padding(name, padding), i)]
		i += 1
	lines += ["%s%s= %i" % (num, get_padding(num, padding), i)]
	return lines

def create_lua_constants_flags(names):
	lines = []
	i = 0
	for name in names:
		lines += ["%s%s= %i  -- %i" % (name, get_padding(name, 27), 1<<i, i)]
		i += 1
	return lines
...
if "lua_constants" in sys.argv: gen_lua_constants = True
...
# LUA NETWORK CONSTANTS
if gen_lua_constants:

	print("-- This file contains all the network constants")
	print("-- (autogenerated, do not edit!)")
	print("")

	for e in network.Enums:
		for l in create_lua_constants(["%s_%s"%(e.name, v) for v in e.values], 'NUM_%sS'%e.name, 24): print(l)
		print("")

	for e in network.Flags:
		for l in create_lua_constants_flags(["%s_%s" % (e.name, v) for v in e.values]): print(l)
		print("")
